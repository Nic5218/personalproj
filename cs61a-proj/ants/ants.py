...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        # place is the entire space where the game takes place
        # place.entrace is non-None value when entrance is constructed
        if self.exit != None:
...
class Ant(Insect):
    ...
    def add_to(self, place): 
        if place.ant is None:
            ...
            if self.can_contain(place.ant): # added ant can contain the original occupying ant
                self.contain_ant(place.ant)  
                place.ant = self # NOTE: self is the added ant
            
            elif place.ant.can_contain(self):   # originally occupied ant can contian the added ant
...
class HarvesterAnt(Ant):
    ...
    name = 'Harvester'
    implemented = True
    # OVERRIDE CLASS ATTRIBUTES HERE (done)
    food_cost = 2
...
class ThrowerAnt(Ant):
    ...
    implemented = True
    damage = 1
    # ADD/OVERRIDE CLASS ATTRIBUTES HERE (done)
    food_cost = 3
    max_range = float('inf')
    min_range = 0
    ...
    def nearest_bee(self, beehive):
        ...
        # iterate through the squares (place), if that square has bee(s), put the list of bees in side bee_selector
        # make a temp_place for ants (not changing ant's position)
        # while loop (while current place is not hive)
        current_place = self.place
        #current_bees = current_place.bees
        place_counter = 0
        
        while current_place != beehive:
            if current_place.bees and self.min_range <= place_counter <= self.max_range:
        ...
        # min_range(from its own place to 3) and max_range (for Long is finite)
...
class LongThrower(ThrowerAnt):
    ...
    food_cost = 2
    # OVERRIDE CLASS ATTRIBUTES HERE
    min_range = 5 # not including its own place
    max_range = float('inf') 
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
    # END Problem 4
...
class HungryAnt(Ant):
    ...
    def __init__(self,health=1,chewing=0, place = None):
        super().__init__(health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        if not self.contained_ant and self.is_container() and not other.is_container():
            return True
        return False
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, gamestate):
    # attack bees in its place, and perform it's guarded ant's action
        if self.contained_ant:
            self.contained_ant.action(gamestate)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        Place.add_insect(self,insect) # call a method with class name
...
class ScubaThrower(ThrowerAnt):
    ...
    def __init__(self, health=1,place=Water):
        self.health = health
        self.place = place
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if not self.true_queen:
            ...
            while temp_place:
                if temp_place.ant:
                    temp_place.ant.buff()
                    if temp_place.ant.is_container() and temp_place.ant.contained_ant:
                        temp_place.ant.contained_ant.buff()
    ...
    def reduce_health(self, amount):
        ...
        # if self.health <= 0 and not self.true_queen:
        #     self.place.remove_insect(self)
        
        # elif self.health <= 0 and self.true_queen:
        #     bees_win()
        super().reduce_health(amount)
        if self.health <= 0 and self.true_queen:
            bees_win()

        # reduce_health method from Insect
        #self.health -= amount
        # if self.health <= 0:
        #     self.death_callback()
        #     self.place.remove_insect(self)
